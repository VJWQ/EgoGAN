...
class ResNet(nn.Module):
    ...
    def forward(self, x, bboxes=None):
        print(x[0].shape)   # torch.Size([1, 3, 8, 224, 224])
        x = self.s1(x)  # torch.Size([1, 64, 8, 56, 56])
        print(x[0].shape)
        # torch.Size([21, 3, 16, 224, 224])
        # torch.Size([21, 64, 16, 56, 56])
        x = self.s2(x)   # torch.Size([1, 256, 8, 56, 56])
        print(x[0].shape)

        for pathway in range(self.num_pathways):
            pool = getattr(self, "pathway{}_pool".format(pathway))
            x[pathway] = pool(x[pathway])
        x = self.s3(x)  # torch.Size([1, 512, 4, 28, 28])
        print(x[0].shape)
        x = self.s4(x)  # torch.Size([1, 1024, 4, 14, 14])
        print(x[0].shape)
        x = self.s5(x)  # torch.Size([1, 2048, 4, 7, 7])    # with temporal dimension: 1. drop 2. temporal conv
        print(x[0].shape)
        if self.enable_detection:
            x = self.head(x, bboxes)
        else:
...
# torch.Size([1, 512, 4, 28, 28])
# torch.Size([1, 1024, 4, 14, 14])
# torch.Size([1, 2048, 4, 7, 7])
# torch.Size([106])
