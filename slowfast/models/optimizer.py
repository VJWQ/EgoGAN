...
def construct_ENDE_optimizer(tup, cfg):
    """
    Construct a stochastic gradient descent or ADAM optimizer with momentum.
    Details can be found in:
    Herbert Robbins, and Sutton Monro. "A stochastic approximation method."
    ...
    #     len(non_bn_parameters), len(bn_params), len(list(model.parameters()))
    # )
    assert len(list(tup[0].parameters())) + len(list(tup[1].parameters())) == len(non_bn_parameters) + len(
        bn_params
    ), "parameter size does not match: {} + {} != {}".format(
        len(non_bn_parameters), len(bn_params), len(list(tup[0].parameters())) + len(list(tup[1].parameters()))
    )
    if cfg.SOLVER.OPTIMIZING_METHOD == "sgd":
        return torch.optim.SGD(
            optim_params,
