...
class Egteamaskopt(torch.utils.data.Dataset):
    def __init__(self, cfg, mode, num_retries=10):
        ...
        logger.info("Constructing EGTEAMASKOPT {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        with g_pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                opt_flow_folder = os.path.join(self._op_path, path)

                imgx = cv2.imread(os.path.join(opt_flow_folder, 'flow_x_00001.jpg'), cv2.IMREAD_GRAYSCALE)
                imgy = cv2.imread(os.path.join(opt_flow_folder, 'flow_y_00001.jpg'), cv2.IMREAD_GRAYSCALE)
                opt_flow = np.concatenate((imgx, imgy), axis=0)
                # 1,3,5,7,9,10,11,12
                opt_idx = ['03', '05', '07', '09', '10', '11', '12']
                for i in range(len(opt_idx)):
                    imgx = cv2.imread(os.path.join(opt_flow_folder, 'flow_x_000'+opt_idx[i]+'.jpg'), cv2.IMREAD_GRAYSCALE)
                    imgy = cv2.imread(os.path.join(opt_flow_folder, 'flow_y_000'+opt_idx[i]+'.jpg'), cv2.IMREAD_GRAYSCALE)
                    opt_flow = np.concatenate((opt_flow, imgx, imgy), axis=0)
                ...
                for idx in range(self._num_clips):
                    ...
                    self._opt_flow.append(opt_flow)
        ...
        ), "Failed to load EGTEAMASKOPT split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing EGTEAMASKOPT dataloader (size: {}) from {}".format(
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            # T H W C -> C T H W.
            frames = frames.permute(3, 0, 1, 2)     # torch.Size([3, 8, 256, 340])

            # fi-m-la
            label = torch.FloatTensor(self._labels[index])
            label = torch.reshape(label, (3, 256, 340))

            optflow = torch.FloatTensor(self._opt_flow[index])
            optflow = torch.reshape(optflow, (2, 8, 256, 340))  # torch.Size([2, 8, 256, 340])
            
            label = torch.unsqueeze(label, 0)   # torch.Size([1, 3, 256, 340])
            tmp = torch.ones([1, 1, 256, 340])
            label = torch.cat((label, tmp), dim=1)  # torch.Size([1, 4, 256, 340])
            label = label.repeat(1,2,1,1)           # torch.Size([1, 8, 256, 340])
            comb = torch.cat((optflow, frames, label), dim=0)   # torch.Size([6, 8, 256, 340])
