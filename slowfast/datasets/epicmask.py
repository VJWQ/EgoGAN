...
class Epicmask(torch.utils.data.Dataset):
    def __init__(self, cfg, mode, num_retries=10):
        ...
        logger.info("Constructing EPICMASK {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load EPICMASK split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing EPICMASK dataloader (size: {}) from {}".format(
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            label = torch.unsqueeze(label, 0)   # torch.Size([1, 2, 256, 456])
            tmp = torch.ones([1, 1, 256, 456])
            label = torch.cat((label, tmp), dim=1)
            label = label.repeat(1,2,1,1)
            comb = torch.cat((frames, label), dim=0)   # torch.Size([1, 8, 256, 456])
