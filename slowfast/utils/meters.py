...
class TestMeter(object):
    ...
    def __init__(
        ...
        # fi/la
        # self.video_preds = torch.zeros((num_videos, num_cls, 1, 224, 224))
        # fi-la
        # self.video_preds = torch.zeros((num_videos, num_cls, 2, 224, 224))
        # fi-m-la
        self.video_preds = torch.zeros((num_videos, num_cls, 3, 224, 224))
        ...
        # fi/la
        # self.video_labels = torch.zeros((num_videos, num_cls, 1, 224, 224))
        # self.video_metas = torch.empty((num_videos, num_cls, 1, 224, 224))
        # self.video_labels = torch.zeros((num_videos, num_cls, 2, 224, 224))
        # self.video_metas = torch.empty((num_videos, num_cls, 2, 224, 224))
        # fi-la
        self.video_labels = torch.zeros((num_videos, num_cls, 3, 224, 224))
        self.video_metas = torch.empty((num_videos, num_cls, 3, 224, 224))
        # fi-m-la
...
class TrainMeter(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        self.loss_total_g = 0.0
        self.loss_d = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_total_d = 0.0
        self.loss_flow = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_total_flow = 0.0
    ...
    def update_stats(self, loss, lr, loss_flow, loss_g, loss_d, mb_size):
        """
        Update the current stats.
        Args:
        ...
        self.loss_total_g += loss_g * mb_size
        self.loss_total_d += loss_d * mb_size
        self.loss_flow.add_value(loss_flow)
        self.loss_total_flow += loss_flow * mb_size
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            "loss": self.loss.get_win_median(),
            "lr": self.lr,
            "loss_flow": self.loss_flow.get_win_median(),
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            ...
            avg_loss_g = self.loss_total_g / self.num_samples
            avg_loss_d = self.loss_total_d / self.num_samples
            avg_loss_flow = self.loss_total_flow / self.num_samples
            stats["loss"] = avg_loss
            stats["loss_flow"] = avg_loss_flow
            stats["loss_g"] = avg_loss_g
            stats["loss_d"] = avg_loss_d
        logging.log_json_stats(stats)
...
class ValMeter(object):
    ...
    def update_stats(self, loss, loss_flow, loss_g, loss_d, mb_size):
        """
        Update the current stats.
        Args:
            top1_err (float): top1 error rate.
